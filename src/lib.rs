...
pub fn insert_text(
    ...
    copy_wait_time_ms: Option<u32>,
    paste_wait_time_ms: Option<u32>,
    release_command_key_before_text_insert: Option<bool>
) {
    let mut enigo = Enigo::new(&Settings::default()).unwrap();

    let arrow_key_to_click_before_insert = arrow_key_to_click_before_insert.unwrap_or(
        String::new()
    );
    ...
    if insert_with_paste {
        ...
        if text.is_empty() {
            ...
            thread::sleep(
                time::Duration::from_millis(
                    u64::from(copy_wait_time_ms.unwrap_or(DEFAULT_PASTE_WAIT_TIME_MS))
                )
            );
            ...
            thread::sleep(
                time::Duration::from_millis(
                    u64::from(paste_wait_time_ms.unwrap_or(DEFAULT_COPY_WAIT_TIME_MS))
                )
            );
        ...
        // In macOS, there is an issue where text insert won't work if the command key is held
        // https://github.com/enigo-rs/enigo/issues/297
        let release_command_key_before_text_insert =
            release_command_key_before_text_insert.unwrap_or(false);
        if release_command_key_before_text_insert {
...
fn paste(enigo: &mut Enigo) {
    let control_or_command_key = if cfg!(target_os = "macos") { Key::Meta } else { Key::Control };
